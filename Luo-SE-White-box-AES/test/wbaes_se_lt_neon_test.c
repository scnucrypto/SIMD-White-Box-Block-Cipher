...
// wbaes_se_lt_neon_context *neon_ctx;
...
// size_t test_wbaes_se_lt_neon_ecb_crypt_loop(size_t size){
//     size_t count = 0;

//     for (count = 0; run && count < 0xffffffffffffffff; count++)
//     {
//         wbaes_se_lt_neon_ecb_encrypt(input, output, size, neon_ctx);
//     }
...
//     neon_ctx = wbaes_se_lt_neon_context_init();
    
//     wbaes_se_lt_neon_gen_table(neon_ctx, key_vector);

//     // wbaes_se_lt_neon_ecb_encrypt(IN, OUT, 16, lt_ctx);
//     printf("\nWBAES_SE_LT_neon:\n");
//     performance_test_enc(test_wbaes_se_lt_neon_ecb_crypt_loop, size, 7, 3);


//     wbaes_se_lt_neon_context_free(neon_ctx);
