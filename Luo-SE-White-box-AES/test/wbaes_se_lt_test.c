...
wbaes_se_lt_context *ctx;
wbaes_se_lt_avx2_context *avx2_ctx;
...
size_t test_wbaes_se_lt_ecb_crypt_loop(size_t size){
    size_t count = 0;
...
size_t test_wbaes_se_lt_avx2_ecb_crypt_loop(size_t size){
    size_t count = 0;

    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbaes_se_lt_avx2_ecb_encrypt(input, output, size, avx2_ctx);
    }
...
int main()
{
    ...
    // ! 注意不能同时测试，会影响性能，每次只测试一个算法
    #if 1
        ctx = wbaes_se_lt_context_init();
        
        wbaes_se_lt_gen_table(ctx, key_vector);

        // wbaes_se_lt_ecb_encrypt(test_in, test_out, 128, lt_ctx);
        printf("\nWBAES_SE_LT:\n");
        wbaes_se_lt_encrypt(IN, OUT, ctx);
        dump_hex(OUT,16);
        // performance_test_enc(test_wbaes_se_lt_ecb_crypt_loop, size, 7, 3);
    
        wbaes_se_lt_context_free(ctx);
    #endif
    
    #if 0
        avx2_ctx = wbaes_se_lt_avx2_context_init();
        ...
        wbaes_se_lt_avx2_context_free(avx2_ctx);
    #endif
    return 0;
}