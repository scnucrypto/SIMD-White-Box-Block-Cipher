...
// static uint8_t L_F4[16][16][16];
// static uint8_t L_E4[16][16][16];

// static uint8_t SR_F4[16][16][16];
// static uint8_t SR_E4[16][16][16];
...
//     wbaes_se_lt_neon_context *ctx = malloc(sizeof(wbaes_se_lt_neon_context));
//     if (ctx == NULL) {
//         goto cleanup;
//     }
//     memset(ctx, 0, sizeof(wbaes_se_lt_neon_context));
//     return ctx;
// cleanup:
//     return NULL;
// }
...
// 	wbaes_se_lt_context aes_key;
//     wbaes_se_lt_gen_table(&aes_key,key);
//     M4 TMP[32][32];
//     MatrixdivM128to4(aes_key.AL_F.Mat,TMP);
//     for (size_t j = 0; j < 16; j++)
//     {
//         GEN_TABLE_DATA_F4(TMP,j,ctx->AL_F_M_F4);
//         GEN_TABLE_DATA_E4(TMP,j,ctx->AL_F_M_E4);
//     }	
//     PUT64(aes_key.AL_F.Vec.V[0],ctx->AL_F_V);
//     PUT64(aes_key.AL_F.Vec.V[1],ctx->AL_F_V+8);

//     MatrixdivM128to4(L_matrix,TMP);
//     for (size_t j = 0; j < 16; j++)
//     {
//         GEN_TABLE_DATA_F4(TMP,j,L_F4);
//         GEN_TABLE_DATA_E4(TMP,j,L_E4);
//     }	
//     MatrixdivM128to4(shiftrows,TMP);
//     for (size_t j = 0; j < 16; j++)
//     {
//         GEN_TABLE_DATA_F4(TMP,j,SR_F4);
//         GEN_TABLE_DATA_E4(TMP,j,SR_E4);
//     }	

// 	for (size_t i = 0; i < 10; i++)
// 	{
// 		MatrixdivM128to4(aes_key.AL_LP[i].Mat,TMP);
//         for (size_t j = 0; j < 16; j++)
//         {
//             GEN_TABLE_DATA_F4(TMP,j,ctx->AL_LP_M_F4[i]);
//             GEN_TABLE_DATA_E4(TMP,j,ctx->AL_LP_M_E4[i]);
//         }	
// 		PUT64(aes_key.AL_LP[i].Vec.V[0],ctx->AL_LP_V[i]);
//         PUT64(aes_key.AL_LP[i].Vec.V[1],ctx->AL_LP_V[i]+8);

//         MatrixdivM128to4(aes_key.AL_LQ[i].Mat,TMP);
//         for (size_t j = 0; j < 16; j++)
//         {
//             GEN_TABLE_DATA_F4(TMP,j,ctx->AL_LQ_M_F4[i]);
//             GEN_TABLE_DATA_E4(TMP,j,ctx->AL_LQ_M_E4[i]);
//         }	
// 		PUT64(aes_key.AL_LQ[i].Vec.V[0],ctx->AL_LQ_V[i]);
//         PUT64(aes_key.AL_LQ[i].Vec.V[1],ctx->AL_LQ_V[i]+8);
// 	}
...
// int wbaes_se_lt_neon_encrypt(const uint8_t IN[256], uint8_t OUT[256], wbaes_se_lt_neon_context *ctx) {
...
//     AFFINE_ENCODING(state1, state1, ctx->AL_F_M_F4, ctx->AL_F_M_E4, ctx->AL_F_V);
...
//         AFFINE_ENCODING(state1, state1, ctx->AL_LP_M_F4[i], ctx->AL_LP_M_E4[i], ctx->AL_LP_V[i]);
// 		AFFINE_ENCODING(state1, state1, L_F4, L_E4, zero);
//         AFFINE_ENCODING(state1, state1, ctx->AL_LQ_M_F4[i], ctx->AL_LQ_M_E4[i], ctx->AL_LQ_V[i]);
// 	}
//     for (size_t j = 0; j < 16; j++)
//     {
//         tmp_8x16 = state1[j].vect_u8;
...
//         state1[j].vect_u8 = veorq_u8(state1[j].vect_u8, tmp1_8x16);
//     }
// 	AFFINE_ENCODING(state1, state1, ctx->AL_LP_M_F4[9], ctx->AL_LP_M_E4[9], ctx->AL_LP_V[9]);
//     AFFINE_ENCODING(state1, state1, SR_F4, SR_E4, zero);
//     AFFINE_ENCODING(state1, state1, ctx->AL_LQ_M_F4[9], ctx->AL_LQ_M_E4[9], ctx->AL_LQ_V[9]);
...
//                 wbaes_se_lt_neon_encrypt(tmp_input, tmp_output, ctx);
