...
size_t test_wbspeck_se_bit_packed_ecb_crypt_loop(size_t size){
    size_t count = 0;

    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbspeck_se_bit_packed_ecb_encrypt(input, output, size);
    }
...
int main()
{
   size_t size[7] = {16, 64, 256, 512, 1024, 8192, 16384};
    ...
    #if 1
        printf("\nWBSPECK_SE_BP:\n");
        performance_test_enc(test_wbspeck_se_bit_packed_ecb_crypt_loop, size, 7, 3);
    #endif
