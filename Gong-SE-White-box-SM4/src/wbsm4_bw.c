#include "wbsm4_bw.h"
// 初始化为1是为了在编译的时候把 数组空间 生成好 放到 可执行文件 里面
// Aff32 SE[4]={1};
// Aff32 FE[4]={1};
// uint32_t TD[32][4][4][256]={1};
// uint32_t TR[32][4][256][256]={1};
...
void wbsm4_bw_encrypt(unsigned char IN[], unsigned char OUT[])
{
    ...
    x[0] = affineU32(SE[0], x[0]);
    x[1] = affineU32(SE[1], x[1]);
    x[2] = affineU32(SE[2], x[2]);
    x[3] = affineU32(SE[3], x[3]);
    ...
    x[35] = affineU32(FE[0], x[35]);
    x[34] = affineU32(FE[1], x[34]);
    x[33] = affineU32(FE[2], x[33]);
    x[32] = affineU32(FE[3], x[32]);
...
void wbsm4_bw_ecb_encrypt(unsigned char *input, size_t len, unsigned char *output){
    assert (len % 16 == 0);
    while (len) {
        wbsm4_bw_encrypt(input,output);
        len -= 16;
        input += 16;
        output += 16;
    }
