...
//     wbcrypto_sm4_lut_setkey_enc(&ctx, test_key);
//     for (count = 0; run && count < 0xffffffffffffffff; count++)
//     {
//         wbcrypto_sm4_lut_ecb_encrypt(&ctx, test_input, size, test_output);
//     }
...
void wbcrypto_sm4_lut_setkey_enc(wbcrypto_sm4_lut_context *ctx, const unsigned char *key)
{
    /*
     * Family Key
     */
...
void wbcrypto_sm4_lut_setkey_dec( wbcrypto_sm4_lut_context *ctx, const unsigned char key[16] )
{
    int i;
    wbcrypto_sm4_lut_setkey_enc(ctx, key);
    for ( i = 0; i < 16; i ++ )
    {
...
#define SM4_RNDS(ks, k0, k1, k2, k3, F)          \
      do {                                   \
...
void wbcrypto_sm4_lut_encrypt(const unsigned char *in, unsigned char *out, const wbcrypto_sm4_lut_context *ks)
{
     ...
     */
    SM4_RNDS(ks,  0,  1,  2,  3, SM4_T_slow);
    SM4_RNDS(ks,  4,  5,  6,  7, SM4_T);
    SM4_RNDS(ks,  8,  9, 10, 11, SM4_T);
    SM4_RNDS(ks, 12, 13, 14, 15, SM4_T);
    SM4_RNDS(ks, 16, 17, 18, 19, SM4_T);
    SM4_RNDS(ks, 20, 21, 22, 23, SM4_T);
    SM4_RNDS(ks, 24, 25, 26, 27, SM4_T);
    SM4_RNDS(ks, 28, 29, 30, 31, SM4_T_slow);
...
void wbcrypto_sm4_lut_decrypt(const unsigned char *in, unsigned char *out, const wbcrypto_sm4_lut_context *ks)
{
    uint32_t B0 = load_u32_be(in, 0);
    uint32_t B1 = load_u32_be(in, 1);
    uint32_t B2 = load_u32_be(in, 2);
    ...
    SM4_RNDS(ks, 31, 30, 29, 28, SM4_T_slow);
    SM4_RNDS(ks, 27, 26, 25, 24, SM4_T);
    SM4_RNDS(ks, 23, 22, 21, 20, SM4_T);
    SM4_RNDS(ks, 19, 18, 17, 16, SM4_T);
    SM4_RNDS(ks, 15, 14, 13, 12, SM4_T);
    SM4_RNDS(ks, 11, 10,  9,  8, SM4_T);
    SM4_RNDS(ks,  7,  6,  5,  4, SM4_T);
    SM4_RNDS(ks,  3,  2,  1,  0, SM4_T_slow);
...
int wbcrypto_sm4_lut_ecb_encrypt(const wbcrypto_sm4_lut_context *ctx, const unsigned char IN[], size_t ilen, unsigned char OUT[])
{ 
    assert (ilen % 16 == 0);
    while (ilen) {
        wbcrypto_sm4_lut_encrypt(IN,OUT,ctx);
        ilen -= 16;
        IN += 16;
        OUT += 16;
    }
...
{
    memset( ctx, 0, sizeof(wbcrypto_sm4_lut_context) );
    if(ctx!=NULL){
        free(ctx);
        ctx=NULL;
    }
