...
size_t test_wbsm4_se_la_ecb_crypt_loop(size_t size){
    size_t count = 0;

    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbsm4_se_la_ecb_encrypt(&ctx, input, size, output);
    }
...
int main() {
    ...
    wbsm4_se_la_gen(&ctx, key_vector);
    ...
    wbsm4_se_la_encrypt(input,output,&ctx);
    printf("\n");
    ...
    printf("\nwbsm4_se_la_ecb:\n");
    performance_test_enc(test_wbsm4_se_la_ecb_crypt_loop, size, 7, 3);
    return 0;
}
