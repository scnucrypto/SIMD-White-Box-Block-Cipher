...
size_t test_wbsm4_bw_ecb_crypt_loop(size_t size){
    size_t count = 0;

    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        wbsm4_bw_ecb_encrypt(input, size, output);
    }
...
int main() {
    ...
    performance_test_enc(test_wbsm4_bw_ecb_crypt_loop, size, 7, 3);
    return 0;
}
