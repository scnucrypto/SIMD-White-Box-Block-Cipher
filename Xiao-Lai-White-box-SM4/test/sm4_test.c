...
size_t test_sm4_ecb_crypt_loop(size_t size){
    size_t count = 0;

    for (count = 0; run && count < 0xffffffffffffffff; count++)
    {
        sm4_crypt_ecb(&ctx,SM4_ENCRYPT,size,input,output);
    }
...
int main() {
    ...
    printf("\nsm4_ecb:\n");
    performance_test_enc(test_sm4_ecb_crypt_loop, size, 7, 3);
